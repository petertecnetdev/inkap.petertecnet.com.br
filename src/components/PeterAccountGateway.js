...
function loadSdk() {
  if (!sdkPromise) sdkPromise = loadScript({ selector: "script[data-peter-ecosystem-sdk]", src: SDK_URL, datasetKey: "peterEcosystemSdk", datasetValue: SDK_VERSION, ready: () => Boolean(customElements.get("peter-ecosystem-launcher")), errorMessage: "Não foi possível carregar o Peter Tecnet Ecosystem SDK." });
  return sdkPromise;
}

function loadInsights() {
  if (!insightsPromise) insightsPromise = loadScript({ selector: "script[data-peter-insights-sdk]", src: INSIGHTS_URL, datasetKey: "peterInsightsSdk", datasetValue: INSIGHTS_VERSION, ready: () => Boolean(customElements.get("peter-insight-chart")), errorMessage: "Não foi possível carregar o Peter Tecnet Insights SDK." });
